...
export function Footer(): React.ReactElement {
  return (
    <footer className="bg-brand-primary text-white border-t border-brand-primary" aria-label="Pie de página institucional">
      <Container size="xl">
        {/* Main Footer Columns */}
        <div className="py-12 sm:py-16 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8 md:gap-12">
          {/* Column 1 — Firm Identity */}
          <div className="space-y-4">
            <div>
              <span className="text-xl font-serif font-bold text-white block tracking-tight">
                {siteConfig.name}
              </span>
              <span className="text-xs font-mono uppercase tracking-widest text-brand-accent font-semibold block mt-1">
                {siteConfig.descriptor}
              </span>
            </div>
            <p className="text-sm text-neutral-300 leading-relaxed max-w-sm">
              Consultoría jurídica y representación procesal con sede en Ciudad Juárez, Chihuahua. 25 años de experiencia en litigio civil, mercantil, familiar, penal y amparo.
            </p>
            <div className="text-xs font-mono text-neutral-400 space-y-1">
              ...
              <p>· {siteConfig.metrics.yearsExperience} años de práctica jurídica</p>
            </div>
          </div>

          {/* Column 2 — Practice Areas */}
          <div className="space-y-4">
            <h3 className="text-xs font-mono uppercase tracking-widest text-brand-accent font-semibold">
              Áreas de Práctica
            </h3>
            ...
            <ul className="space-y-2.5 text-sm" role="list">
              ...
              <li>
                ...
                >
                  ...
                  <span className="px-1 py-0.2 text-[9px] font-mono uppercase bg-brand-accent/20 text-brand-accent rounded-sm">
                    Online
                  </span>
                </Link>
              </li>
          ...
          <div className="space-y-4">
            ...
            <div className="text-sm text-neutral-300 space-y-3">
              <div>
                <span className="text-xs text-neutral-400 block">Teléfono / WhatsApp:</span>
                <a
                  href={siteConfig.contact.phoneHref}
                  className="text-white hover:text-brand-accent font-mono transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-accent rounded-sm"
                >
                  {siteConfig.contact.phoneDisplay}
                </a>
              </div>
              <div>
                <span className="text-xs text-neutral-400 block">Horario de Atención:</span>
                <span className="text-white font-mono text-xs">{siteConfig.contact.hours}</span>
              </div>
              <div>
                <span className="text-xs text-neutral-400 block">Ubicación:</span>
                <span className="text-white text-xs">{siteConfig.location.city}, {siteConfig.location.state}</span>
              </div>
              <div className="pt-2">
                <Link
                  href="/agenda"
                  className="inline-flex items-center justify-center px-4 py-2 border border-white/30 hover:border-brand-accent text-white hover:text-brand-accent text-xs font-medium rounded-sm transition-colors"
                >
        ...
        {/* Legal Bottom Bar */}
        <div className="border-t border-white/10 py-6 flex flex-col sm:flex-row items-center justify-between gap-4 text-xs text-neutral-400">
